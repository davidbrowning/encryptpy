...
# I tried to use the algorithm I had written for euclid_numbers_py3.py, 
...
# use euclid_numbers_py3.prime_factor(n, factor_list)

code_separator = euclid_numbers_py3.euclid_number(33)
...
def pascal_tri_row_sum(n):
    answer = 2
    if(euclid_numbers_py3.is_prime(n)):
        k = pascal_tri_row(n)
        for item in k:
            if(item != 1):
                answer += (item/n)
...
def decrypt_text(txt):
    ...
    #print num_list
    message = ''
    for item in range(len(num_list)/2):
